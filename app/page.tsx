...
import { projects, stack } from "@/src/constants/projects";
const cv = "/cv.pdf";
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col w-full items-center px-4 py-8 sm:px-8 bg-(--main-content-bg-color)">
      <section className="flex flex-col items-center w-full max-w-3xl gap-6 md:px-9">
        <div className="flex items-center justify-center">
          <Image
            src={profile}
            alt="logo"
            className="w-20 h-20 rounded-full border-2 border-(--link-hover-color) object-cover bg-(--nav-bar-bg-color)"
          />
          <div className="flex flex-col ml-4 justify-center">
            <h6 className="text-(--text-color) text-xl font-bold">DANIEL</h6>
            <h6 className="text-(--text-color) text-base">@Daniel.dev</h6>
          </div>
        </div>

        <div className="flex gap-13 text-sm text-(--text-color)">
          <span className="flex">
            <p className="mr-1 text-(--main-color-small-info)">+{years}</p>
            Años como desarrollador
          </span>
          <span className="flex">
            <p className="mr-1 text-(--main-color-small-info)">+20</p>
            proyectos realizados
          </span>
        </div>
      </section>

      <p className="mt-3 max-w-3xl text-center md:text-left text-sm text-(--text-color)">
      ...
      <div className="flex gap-4 mt-4">
        <Link
          target="_blank"
          href={"https://github.com/DANIElPEZ"}
        ...
        <Link
          target="_blank"
          href={"https://www.linkedin.com/in/daniel-g-dev"}
          style={{ clipPath: "url(#squircleClip)" }}
          className="w-12 h-12 bg-[#172c4d] rounded-xl flex items-center justify-center shadow-lg border border-gray-600/50 cursor-pointer transform transition hover:scale-110 hover:-translate-y-2"
        >
          <svg
      ...
      <section className="w-full max-w-6xl mt-1">
        <h3 className="text-2xl font-medium text-(--text-color) mb-3">
          Proyectos
        </h3>
      ...
      `}
                ...
                <div className="flex flex-wrap gap-2 items-center justify-center">
                  {p.imagenes.map((img, index) => (
                    <img
                      ...
                      className="max-h-50 object-contain rounded-lg shadow-md"
                    />
                  ))}
                </div>
              </div>
        ...
        <h3 className="text-2xl font-medium text-(--text-color) my-4">
          Stack tecnológico
        </h3>
